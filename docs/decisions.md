...
- **Persistent mutation is a locked read-modify-replace transaction.** Canonical absolute paths identify a bounded per-cache exclusive sidecar lock. The empty lock file is retained: deleting it could let a new opener lock a different inode from an existing waiter. Serialization and durable file flushing complete before atomic replacement on the target filesystem. Temporary storage is owner-only before content is written, and existing restrictive Unix modes or Windows ACLs survive replacement. Failures preserve the previous live file and remain fail-soft to enrichment callers.
- **Evidence refresh is monotonic within its validity window.** A current malicious verdict for the exact lookup survives weaker evidence while an absent canonical relationship becomes known; the binding is learned onto retained evidence instead of discarding it. A fresh unbound malicious result may inherit an already-established binding for that exact lookup, so concurrent arrival order cannot erase it. Expired and future-dated evidence does not suppress a current replacement. Weaker null bindings and every contradictory nonempty binding still leave live cache bytes unchanged.
...
- **Session evidence expires independently of persistent storage.** The in-memory VirusTotal cache is always active but never written to disk. `Clean` and `Unknown` remain valid through 7 days, `Malicious` through 90 days, and future timestamps are rejected. `Unknown` avoids repeated quota-consuming absence queries; `Unavailable` is not cached so a transient provider or validation failure can recover immediately.
...
- **Canonical cache reuse requires an explicit identity binding.** A provider-derived SHA-256 is persisted and reused only when VirusTotal's alias for the input hash matches that input case-insensitively. Loose cross-hash searches are forbidden because they could attach another file's evidence to the lookup.
- **The EPM cascade has one SHA-1 identity boundary.** `Get-FileReputation` rejects MD5, SHA-256, malformed, and non-hexadecimal inputs before provider access because its downstream evidence starts from the EPM SHA-1. Direct `Get-VtFileReport` remains a separate provider command and continues to accept MD5, SHA-1, and SHA-256 identifiers.
- **MalwareBazaar evidence is bound to one unambiguous returned SHA-1.** An `ok` response is malicious only when exactly one record matches the requested SHA-1 case-insensitively. Missing, mismatched, or duplicate matching identities are unavailable evidence, and provider payloads are not exposed through the public detail.
- **EPM grouping canonicalizes hash case, not publisher case.** Binary groups and per-user distinct-binary counts use uppercase SHA-1 identity. This merges casing variants deterministically without broadening this task into publisher-name normalization.
...
60. **Detect literal secrets at unresolved call-operator sinks without disclosing them.** The repository scanner rejects a non-empty string literal bound to `ApiKey` or `AuthKey` when an ampersand invocation cannot be resolved, because the dynamic target prevents a narrower connection-command check. Findings contain only the file, line, and stable category so the scanner cannot echo the detected value into test or CI output. Variables and runtime-computed expressions are not evidence of a committed literal, and exact synthetic fixture exemptions apply only under `tests/`. This bounded rule is not complete PowerShell data-flow analysis, does not replace Gitleaks, and makes no real-tenant compatibility claim.
