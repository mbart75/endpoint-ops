...
Persistent updates serialize through a per-cache sidecar lock with a five-second timeout. Each update re-reads under that lock and atomically replaces the cache with a fully flushed temporary file on the same filesystem. The temporary file is owner-only before cache content is written; an existing restrictive Unix mode or Windows ACL is preserved across replacement. The empty `.lock` file is retained to avoid a waiter locking a different inode. Fresh malicious evidence for the exact lookup survives weaker refreshes and a later canonical-binding discovery. Established nonempty canonical bindings cannot be removed or replaced by a contradictory update.
